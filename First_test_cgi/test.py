...
q="SELECT e.emp_no,e.first_name,e.last_name,e.gender,e.email_id,s.salary,s.FROM_date,s.to_date FROM employees e JOIN salaries s on s.emp_no=e.emp_no WHERE e.emp_no=10004;"
Dlst,Hlst= DQ.Database_Connection().query_data(q)
#QF.Query_Extractor().writeToFile('obne.html',htmcode)

htmcode=HT.CreateHTMLTable().createHTML(Dlst,Hlst)
print htmcode
